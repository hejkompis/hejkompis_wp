				...
				<div id="post-<?php the_ID(); ?>" <?php post_class($layout); ?> data-post_ID="<?php the_ID(); ?>" data-nonce="<?php echo wp_create_nonce('like_post'); ?>">
					...
					<?php 
						...
						foreach($post_tags as $post_tag) {
							if($post_tag->slug == 'spotify_favourite' || $post_tag->slug == 'spotify_album') {
								$spotify_tag = true;
							} 
						}						
					...
					<?php
						if (has_post_thumbnail(get_the_ID()) && ( $layout == "single" || $layout == "full") && isset($inspire_options_post['show_featured']) && !$spotify_tag) {
						?>
							<div class='post-img'>
								<div class='flexslider flexslider_single'>
									<ul class="slides">
